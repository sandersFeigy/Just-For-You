...
function priceError(price, priceNeded) {
  ...
  else if (priceNeded[0] > Price || priceNeded[1] < Price) {
    return "המחיר אינו בטווח הדרוש,נסה/י שנית";
  }
  return null;
}
...
function sellFurmat(ValuesOfInputs) { 
  if ((ValuesOfInputs.id == undefined || ValuesOfInputs.sellerId == undefined
    ...
    ValuesOfInputs.src == undefined) ||
    (ValuesOfInputs.id == null || ValuesOfInputs.sellerId == null  
...
function valuesErrors(ValuesOfInputs, buyItem) {
  ...
  const errorPrice = priceError(ValuesOfInputs.price, buyItem.range);
